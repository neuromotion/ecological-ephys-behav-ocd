...
% Acc_times,x, y, z
rcs_accel = csvread([loaddir,'004_accel.csv']);
...
% lfp_times
lfp_all = csvread([loaddir,'004_lfp.csv']);
lfp_ch1 = lfp_all(:,2);
lfp_ch2 = lfp_all(:,4);
...
starttime = datetime(2020,9,18,15,01,13,'TimeZone','America/Chicago');
...
timespan = seconds([datetime(2020,9,18,15,38,25,'TimeZone','America/Chicago'),datetime(2020,9,18,15,38,35,'TimeZone','America/Chicago')]-starttime);
...
%% save
img_name=[savedir,'CBT_screenshot004.png'];
saveas(fig,img_name);
img_name=[savedir,'CBT_screenshot004.svg'];
saveas(fig,img_name);
